"""Integration admin CRUD migrated to Litestar :20002 in wave 66.

The Slack slash-command webhook (`/api/integrations/slack/command`) stays on
Flask until the dedicated subprocess/webhook wave because it spawns a
background thread, performs HMAC verification, and posts back via httpx —
all of which deserve a structured Litestar treatment together with
github_webhook and oauth_callback.
"""
...
tag = Tag(name="integrations", description="Migrated to Litestar")
...
slack_command_bp = APIBlueprint(
    ...
    abp_tags=[slack_command_tag],
)


@slack_command_bp.post("/slack/command")
def slack_slash_command():
    """Inbound Slack slash command (`/agented`) — verifies HMAC and dispatches."""
    ...
    AuditLogService.log(
        ...
        entity_id="slack",
        outcome="received",
        details={"command": command, "text": text, "user_id": user_id, "channel_id": channel_id},
    )

    trigger_name, target, parse_error = _parse_run_command(text)
    if parse_error:
        return jsonify({"response_type": "ephemeral", "text": parse_error}), 200
        ...
        return jsonify(
            {
                ...
                "text": (
                    ...
                    "Use `/agented run <trigger-name> on <target>`."
                ),
            }
        ), 200
    ...
    return jsonify(
        {"response_type": "ephemeral", "text": f"Starting {trigger_name} on {target}..."}
    ), 200
...
def _dispatch_slash_command_execution(trigger: dict, target: str, response_url: str, user_id: str):
    ...
    if response_url:
        try:
            httpx.post(
                response_url,
                json={"response_type": "ephemeral", "text": result_text},
                timeout=10.0,
            )
        except Exception as e:
