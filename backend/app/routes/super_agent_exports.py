...
def export_super_agent():
    ...
    if not super_agent_id:
        return error_response("BAD_REQUEST", "super_agent_id is required", HTTPStatus.BAD_REQUEST)

    export_format = data.get("export_format", "zip")
    if export_format not in ("directory", "zip"):
        return error_response(
            "BAD_REQUEST", "export_format must be 'directory' or 'zip'", HTTPStatus.BAD_REQUEST
        )
...
def validate_super_agent_package():
    ...
    try:
        ...
        if not manifest_path.exists():
            return error_response("NOT_FOUND", "manifest.json not found", HTTPStatus.NOT_FOUND)
