"""Topology strategy functions for team execution.

Each function implements a different multi-agent orchestration pattern.
They receive a `tracker` (TeamExecutionTracker) and `run_agent` (callable)
as parameters instead of being bound to a class.

run_agent signature:
    run_agent(team, agent_id, message, event, trigger_type, working_directory) -> (execution_id, output)
"""
...
def get_agent_from_member(team: dict, agent_id: str) -> Optional[dict]:
...
def execute_parallel(
    ...
    *,
    ...
    def _run_one(aid) -> None:
        eid, _ = run_agent(team, aid, message, event, trigger_type, working_directory)
        if eid:
            with exec_lock:
...
def execute_coordinator(
    ...
    *,
    ...
    coord_eid, coord_output = run_agent(
    ...
    def _run_worker(aid) -> None:
        eid, _ = run_agent(team, aid, worker_message, event, trigger_type, working_directory)
        if eid:
            with exec_lock:
...
def execute_generator_critic(
    ...
    *,
    ...
    for iteration in range(max_iterations):
        ...
        critic_eid, critic_output = run_agent(
...
def execute_hierarchical(
    ...
    *,
    ...
    return _execute_hierarchical_recursive(
        team,
        adjacency,
        member_id_to_entity_id,
        lead_member_id,
...
def _execute_hierarchical_recursive(
    ...
    *,
    ...
    for child_member_id in children:
        child_ids = _execute_hierarchical_recursive(
            team,
            adjacency,
            member_id_to_entity_id,
            child_member_id,
...
def execute_human_in_loop(
    ...
    *,
    ...
    team_exec_id = tracker.find_exec_id_for_team(team.get("id", ""))
    ...
    for agent_id in order:
        ...
        if agent_id in approval_nodes:
            ...
            if team_exec_id:
                tracker.set_pending_approval(
                    team_exec_id, agent_id, approval_timeout, approval_event
                )
            ...
            if not approved:
                ...
                if team_exec_id:
                    tracker.set_approval_timeout(team_exec_id)
                return execution_ids
...
def execute_composite(
    ...
    *,
    ...
    for i, sub_group in enumerate(sub_groups):
        ...
        kwargs = {"run_agent": run_agent}
        if sub_topology in ("human_in_loop",):
