...
"""
    ...
    def _safe_segment(segment: str) -> str:
        """Sanitize a single path segment for safe filesystem materialization.

        [04.H1] Restrict to ``[A-Za-z0-9._-]`` and reject empty / dot /
        dot-dot segments so a crafted skill or team name cannot escape the
        intended base directory via ``/`` or ``..`` traversal.
        """
        cleaned = re.sub(r"[^A-Za-z0-9._-]", "-", str(segment)).strip("-")
    ...
    def _assert_within(base: str, target: str) -> None:
        """Assert ``target`` resolves inside ``base`` (defence in depth) [04.H1]."""
    ...
    def _generate_harness_files(cls, project: dict, clone_path: str) -> dict:
        ...
        if teams:
            ...
            for team in teams:
                ...
                # [04.H1] Sanitize segment + assert containment before write.
                safe_name = cls._safe_segment(safe_name)

                team_folder = os.path.join(teams_path, safe_name)
                cls._assert_within(teams_path, team_folder)
        ...
        if all_skills:
            ...
            for skill in all_skills:
                # [04.H1] Sanitize skill name + assert containment before write.
                safe_skill = cls._safe_segment(skill)
                skill_folder = os.path.join(skills_path, safe_skill)
                cls._assert_within(skills_path, skill_folder)
                os.makedirs(skill_folder, exist_ok=True)
                skill_md_path = os.path.join(skill_folder, "SKILL.md")
                if not os.path.exists(skill_md_path):
                    with open(skill_md_path, "w") as f:
                        f.write(f"---\nname: {skill}\n---\n\n# {skill}\n\nSkill placeholder.\n")
                    files_created.append(f".claude/skills/{safe_skill}/SKILL.md")
                    counts["skills"] += 1
