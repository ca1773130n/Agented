...
def get_all_rules(
    ...
    with get_connection() as conn:
        params: list = []
        if project_id:
            sql = "SELECT * FROM rules WHERE (project_id = ? OR project_id IS NULL)"
...
def count_rules(project_id: Optional[str] = None, search: Optional[str] = None) -> int:
    """Count total number of rules, optionally filtered by project."""
    with get_connection() as conn:
        if project_id:
            sql = "SELECT COUNT(*) FROM rules WHERE (project_id = ? OR project_id IS NULL)"
            params: list = [project_id]
        else:
            sql = "SELECT COUNT(*) FROM rules WHERE 1=1"
            params = []
        if search:
            sql += " AND (name LIKE ? OR description LIKE ?)"
            like = f"%{search}%"
