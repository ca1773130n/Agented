...
class TestCLI:
    ...
    def test_main_emits_stable_schema_for_failed_targets(self, tmp_path, monkeypatch, capsys):
        ...
        expected_keys = {
            "label", "source", "ok", "snapshot",
            "size_bytes", "remote_synced", "retained_removed", "error",
        }
        for entry in summary["targets"]:
            assert set(entry.keys()) == expected_keys, (
                f"target entry missing/extra keys: {set(entry.keys())} "
                f"!= {expected_keys}"
            )
...
class TestConcurrentWriter:
    ...
    def test_snapshot_under_concurrent_writer_is_consistent(self, tmp_path):
        ...
        try:
            ...
            dest_dir = tmp_path / "snaps"
            snap = snapshot_one(
                "live", src, dest_dir, timestamp="2026-01-01T00-00-00Z",
            )
        finally:
            stop.set()
            wt.join(timeout=5.0)
