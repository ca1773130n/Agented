...
def test_mine_patterns_apply_argv(isolated_db):
    ...
    with p1, p2:
        runner.mine_patterns(
            "p", "/cwd", apply=True, min_occurrences=5, effect_size=0.3, fdr_q=0.05
        )
    assert cap["cmd"] == [
        "gd",
        "patterns",
...
def test_promote_suggestion_argv_and_parse(isolated_db):
    cap = {}
    out = json.dumps(
        {"promoted": "refactor-rate", "heuristic": "h", "genome_path": ".planning/GENOME.md"}
    )
...
def test_upsert_genome_suggestions_full_replace(isolated_db):
    ...
    b = upsert_genome_suggestions(
        project_id=pid,
        reflections_scanned=2,
        applied=True,
