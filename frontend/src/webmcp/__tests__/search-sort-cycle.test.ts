...
function createSearchableToolMap(domain: string) {
  ...
  tools.set(`hive_${domain}_perform_search`, {
    inputSchema: {
      type: 'object',
  ...
  if (domain === 'agents') {
    tools.set(`hive_${domain}_perform_sort`, {
      inputSchema: {
        type: 'object',
        properties: {
          field: { type: 'string' },
...
describe('search-sort-cycle', () => {
  for (const domain of SEARCHABLE_DOMAINS) {
    it(`${domain}: search filters results and state reflects query`, async () => {
      ...
      const initial = await invokeTool(`hive_${domain}_get_list_state`, {}, tools);
      ...
      const searchResult = await invokeTool(`hive_${domain}_perform_search`, { query: 'alpha' }, tools);
      ...
      const afterSearch = await invokeTool(`hive_${domain}_get_list_state`, {}, tools);
      ...
      // Step 4: Clear search
      await invokeTool(`hive_${domain}_perform_search`, { query: '' }, tools);
      const afterClear = await invokeTool(`hive_${domain}_get_list_state`, {}, tools);
  ...
  it('agents: sort changes field and order in state', async () => {
    ...
    // Initial state
    const initial = await invokeTool('hive_agents_get_list_state', {}, tools);
    const initialData = JSON.parse((initial.data as any).content[0].text);
    expect(initialData.sortField).toBe('name');
    expect(initialData.sortOrder).toBe('asc');

    // Sort by date desc
    const sortResult = await invokeTool('hive_agents_perform_sort', { field: 'created_at', order: 'desc' }, tools);
    expect(sortResult.success).toBe(true);

    // Verify
    const after = await invokeTool('hive_agents_get_list_state', {}, tools);
  ...
  it('search with empty string resets filter', async () => {
    ...
    await invokeTool('hive_teams_perform_search', { query: 'test' }, tools);
    const filtered = await invokeTool('hive_teams_get_list_state', {}, tools);
    expect(JSON.parse((filtered.data as any).content[0].text).itemCount).toBe(1);

    await invokeTool('hive_teams_perform_search', { query: '' }, tools);
    const all = await invokeTool('hive_teams_get_list_state', {}, tools);
