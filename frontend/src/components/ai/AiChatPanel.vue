...
const props = withDefaults(
  defineProps<{
    /**
     * Caller-managed state. Required by design wizards (Plugin / Rule / Hook /
     * Command / Sketch / Workflow / Skill / Agent / ProjectManagement) that
     * own the conversation themselves. Optional with empty defaults so two
     * stripped-down "smart-chat fallback" call sites (AIBackendsPage,
     * SuperAgentPlayground) can omit them — those pages migrated to a thin
     * prop set during 2026-04 and haven't been re-wired to caller-managed
     * mode. v0.5.6+ should either revert those pages to caller-managed or
     * extract a separate AiChatPanelManaged variant.
     */
