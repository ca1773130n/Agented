...
must_haves:
  truths:
    ...
    - "Session-completion hook in ProjectSessionManager triggers GrdSyncService.sync_project() BEFORE broadcasting complete event, guarded by grd_init_status check to skip non-GRD sessions"
    - "Session-completion hook calls GrdPlanningService.unregister_session() to clean up in-memory planning session tracker"
...
NOTE: 01-01-SUMMARY.md is omitted from execution_context since it may not exist yet if Plan 01-01's <output> step was skipped. The executor should read it if available, but must not fail if it is absent. All needed context about GrdPlanningService is available from reading the actual source file directly.
</execution_context>
...
<task type="auto">
  ...
  <action>
    ...
    **Step 3: Wire sync hook into ProjectSessionManager** (`backend/app/services/project_session_manager.py`)
    ...
    - Add a **guarded** sync trigger that only runs for projects with GRD initialized (to avoid unnecessary overhead on agent runs, team spawns, and other non-GRD sessions):
      ```python
      ...
      try:
          ...
          if project_id:
              ...
              # Only sync if this project has GRD initialized (avoids overhead on non-GRD sessions)
              from ..database import get_project
              project = get_project(project_id)
      ...
      ```
    - The guard checks `grd_init_status` to skip sync for projects that have no GRD planning (`grd_init_status='none'` or `'failed'`). This prevents unnecessary DB reads and filesystem scans on every session completion (agent runs, team spawns, management sessions).
    - Also calls `GrdPlanningService.unregister_session(session_id)` to clean up the in-memory planning session tracker (added in Plan 01-01 revision).
  ...
  <done>
    ...
    ProjectSessionManager._handle_session_exit() calls sync before broadcasting complete event, guarded by grd_init_status check (only runs for GRD-initialized projects). Also calls GrdPlanningService.unregister_session() to clean up the planning session tracker.
