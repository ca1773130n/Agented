...
<task type="auto">
  ...
  <action>
    ...
    **Changes to styles (scoped):**
    ...
    Also add a polling mechanism: if `grdInitStatus === 'initializing'`, set up a 5-second interval to re-fetch status until it changes to `ready` or `failed`:
    ```typescript
    let initPollInterval: ReturnType<typeof setInterval> | null = null;

    watch(grdInitStatus, (newVal) => {
    ...
    If `onUnmounted` is not already imported, add it to the vue imports.
  </action>
  <verify>
    ...
    **Component: GrdSettings.vue** (`frontend/src/components/settings/GrdSettings.vue`)
    ...
    async function loadSettings() {
      try {
        ...
        autoInitEnabled.value = settings['grd.auto_init_enabled'] !== 'false';
        syncOnSessionComplete.value = settings['grd.sync_on_complete'] !== 'false';
        defaultVerificationLevel.value = settings['grd.default_verification_level'] || 'proxy';
      } catch (e) {
