...
<task type="auto">
  <name>Task 2: Create PlanningCommandBar and extend MilestoneOverview with phase action buttons</name>
  <files>frontend/src/components/grd/PlanningCommandBar.vue, frontend/src/components/grd/MilestoneOverview.vue</files>
  <action>
    ...
    **Component 2: Extend MilestoneOverview.vue** (`frontend/src/components/grd/MilestoneOverview.vue`)
    ...
    2. In the phase card rendering (the section that iterates over phases), add a small actions area with 3 compact buttons per phase:
       ...
       - "Research" → emits `phaseCommand(phase.phase_number, 'survey')`
  ...
  <verify>
    ...
    - MilestoneOverview shows milestone data with phase cards and hover action buttons
    - PlanningCommandBar shows 4 command groups with 17 command buttons
    (Level 2: Proxy — UI renders and commands dispatch)
  </verify>
  <done>
    PlanningCommandBar renders 17 GRD commands in 4 categories. MilestoneOverview has per-phase action buttons (Discuss, Plan, Research using `survey` command). Build passes.
  </done>
</task>

<task type="auto">
  <name>Task 3: Create PlanningSessionPanel with markdown rendering and question widgets</name>
  <files>frontend/src/components/grd/PlanningSessionPanel.vue</files>
  <action>
    Create `frontend/src/components/grd/PlanningSessionPanel.vue` — the AI session output panel with markdown rendering and interactive question widgets.
  ...
  <verify>
    ...
    Navigate to /projects/{id}/planning in dev and invoke a command:
    - Session panel appears on right side with status bar
    - Streaming output renders as markdown
    - Question widgets appear when AI asks interactive questions
    (Level 2: Proxy — panel renders with streaming output)
  </verify>
  <done>
    PlanningSessionPanel renders streaming output as markdown with question widgets (select, multiselect, text, password). Status bar shows session state. Stop and Clear action buttons work. Build passes.
  </done>
</task>
