...
must_haves:
  truths:
    ...
    - "curl -I http://localhost:20000/health/liveness returns all four security headers on the first request after server startup (proving extensions are active before first route serves)"
    - "app.extensions['limiter'] is accessible from blueprint code and is a Limiter instance (proving downstream plans can apply per-blueprint limits)"
...
<task type="auto">
  ...
  <verify>
    Start the backend (`just dev-backend`) and run:
    ```
